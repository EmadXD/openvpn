...
def setup_install_packages():
    ...
    run_cmd("sudo apt install -y wget git make ipset shadowsocks-libev build-essential")

    # نصب Go
    run_cmd("wget https://go.dev/dl/go1.23.1.linux-amd64.tar.gz")
    run_cmd("sudo rm -rf /usr/local/go")
    run_cmd("sudo tar -C /usr/local -xzf go1.23.1.linux-amd64.tar.gz")

    # اضافه کردن Go به PATH
    bashrc_line = 'export PATH=$PATH:/usr/local/go/bin'
    run_cmd(f'grep -qxF "{bashrc_line}" ~/.bashrc || echo "{bashrc_line}" >> ~/.bashrc')
    run_cmd("export PATH=$PATH:/usr/local/go/bin")

    # کلون و ساخت tun2socks
    if not os.path.exists("tun2socks"):
...
def setup_iptables_fwmark():
    ...
    run_cmd(f"sudo iptables -t mangle -A PREROUTING -s {VPN_SUBNET} -m set --match-set {IPSET_NAME} dst -j MARK --set-mark 1")

def setup_tun2socks_routing():
    run_cmd(f"grep -q '^{PROXY_TABLE} tun2socks' /etc/iproute2/rt_tables || echo '{PROXY_TABLE} tun2socks' | sudo tee -a /etc/iproute2/rt_tables")
